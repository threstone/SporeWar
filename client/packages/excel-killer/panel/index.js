            ...
            methods: {
                ...
                _importJsonCfg(typeDir) {
                    ...
                    if (importPath.indexOf("db://assets") >= 0) {
                        ...
                        this._addLog("一共导入脚本数量: " + res.length + importPath);
                        for (let i = 0, len = res.length; i < len; i++) {
                            let fileinfo = path.join(saveFileFullPath1, res[i]);
                            fs.copyFileSync(fileinfo, path.join(this.importProjectCfgPath, res[i]));
                        }
                        Editor.remote.assetdb.refresh(importPath);
                    } else {
                ...
                onBtnClickOpenJsExportPath() {
                    ...
                    if (fs.existsSync(saveFileFullPath2)) {
                        ...
                        // let files = Globby.sync(pattern);
                        this._addLog("一共导入脚本数量: " + res.length);
                        for (let i = 0, len = res.length; i < len; i++) {
                            let fileinfo = path.join(saveFileFullPath1, res[i]);
                            fs.copyFileSync(fileinfo, path.join(saveFileFullPath2, res[i]));
                        }
                ...
                _getJavaScriptSaveData(excelData, itemSheet, isClient) {
                    ...
                    let sheetDefineData = {};
                    let sheetImportData = {};
                    let useArr = types[0].startsWith("#");
                    let sheetClassData = "{";
                    for (let j = 0; j < title.length; j++) {
                        let key = title[j];
                        let type = types[j];
                        let exportType = target[j];
                        let canExport = false;
                        // Editor.log(key, exportType)
                        if (isClient && exportType.indexOf('c') !== -1) {
                            canExport = true;
                        } else if (!isClient && exportType.indexOf('s') !== -1) {
                            canExport = true;
                        }
                    ...
                    sheetClassData += "}";
                    if (useArr) {
                        sheetClassData = `{arr:${sheetClassData}[]}`;
                    }
                    let typeNum = 0;
                    types.forEach(value => {
                        if (!value.startsWith("?")) {
                ...
                _getJsonSaveData(excelData, itemSheet, isClient) {
                    ...
                    let types = excelData[3];
                    let ret = null;
                    let useFormat1 = types[0].startsWith("#");//#开头类型为数组
                    ...
                    for (let i = 4; i < excelData.length; i++) {
                        ...
                        if (canExport) {
                            let key = lineData[0].toString();
                            if (useFormat1) {

                                if (!saveData2[key]) {
                                    saveData2[key] = { arr: [] }
                                }
                                saveData2[lineData[0].toString()].arr.push(saveLineData);
                            } else {
                                saveData2[key] = saveLineData;
                            }
                        }
                    }
                    ret = saveData2;
                        ...
                        if (this.isExportTs) {
                            ...
                            let writeFileTs = function (savePath, isClient) {
                                ...
                                if (Object.keys(sheetJsData).length > 0) {
                                    ...
                                    // this._addLog( JSON.stringify(sheetJsData) );
                                    let letOBJ = JSON.parse(JSON.stringify(sheetJsData));
                                    ...
                                    this._onSaveTypeScriptCfgFile(fileNameFullPath, itemSheet.sheet, sheetJsData, letOBJ, sheetClassData, sheetImportData);
                                }
                            }.bind(this);
                ...
                _onSaveTypeScriptCfgFile(saveFileFullPath, className, cfgData, defineData, classData, importData) {
                    ...
                    // Editor.log("className:", JSON.stringify(defineData));

                    let saveStr = TemplateReader.replace(/@TypeName/g, className)
                        .replace(/@ClassDefine/g, classData).replace(/@Import/g, importStr);

                    if (JSON.stringify(defineData).indexOf("define") >= 0) {
                        // let defineStr = "export const " + className + "Define = " + JSON.stringify(defineData, null, 4);

                        let str = "{\r";
                        for (const key in defineData) {
                            let item = defineData[key];
                            if (!item.define) continue;
                            if (str.length > 4) str += `,\r`;
                            let desc = `${item.name} ` + (item.tip ? "(" + item.tip + ")" : "");
                            str += `    /** ${desc} */ ${item.define} : ${item.id} `;
                        }
                        str += "\r}";

                        // 转换成枚举类型
                        // let defineStr = ("export enum " + className + "Define  " + JSON.stringify(defineData, null, 4)).replace(/:/g, "=").replace(/"/g, "");
                        let defineStr = ("export enum " + className + "Define  " + str).replace(/:/g, "=").replace(/"/g, "");
                ...
                _onSaveTypeScriptCatalogFile(saveFileFullPath, classData) {
                    ...
                    let catalogBuilder = TemplateCatalog.replace(/@Import/g, importBuilder)
                        .replace(/@Property/g, propertyBuilder).replace(/@JsPro/g, KeyToJsMap).replace(/@JsonPro/g, KeyToJsonMap);
